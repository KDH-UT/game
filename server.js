...
io.on('connection', (socket) => {
    ...
    if (action === 'TOGGLE_SWITCH') {
      ...
      if (entry) {
        if (entry.isCorrect) {
          ...
          io.to(entry.playerId).emit('answer-rejected');
        } else {
          entry.isCorrect = true;
          gameState.teamScores[teamNum] += entry.score;
          io.to(entry.playerId).emit('answer-approved', { score: entry.score });
        }
        io.to(roomCode).emit('leaderboard-update', { teamsCount: gameState.teamsCount, teamScores: gameState.teamScores });
      }
    } else if (action === 'APPROVE_HINT') {
      const { playerId } = data;
      if (gameState.players[playerId]) gameState.players[playerId].isApproved = true;
    } else if (action === 'REJECT_HINT') {
      const { playerId } = data;
      const p = gameState.players[playerId];
      if (p) {
        p.hint = ''; p.hintDuration = null; p.isApproved = false;
        io.to(playerId).emit('hint-rejected');
      }
    } else if (action === 'KICK_MASTER') {
      const { masterId } = data;
      if (gameState.masters[masterId]) {
        delete gameState.masters[masterId];
        io.to(masterId).emit('master-kicked', '방장에 의해 퇴출되었습니다!');
        io.sockets.sockets.get(masterId)?.leave(roomCode);
      }
    }
  ...
  socket.on('answer-submit', (data) => {
    ...
    if (player) {
      ...
      gameState.teamHistories[player.team].push({
  ...
  socket.on('disconnect', () => {
    for (const roomCode in rooms) {
      const gameState = rooms[roomCode];
      if (gameState.masters && gameState.masters[socket.id]) {
        delete gameState.masters[socket.id];
      }
      removePlayerFromTeams(gameState, socket.id);
